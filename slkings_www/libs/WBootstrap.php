...
class WBootstrap {
    ...
    private $arr_tail = Array("gif","jpg","png","hwp","doc","docx","xls","txt","xlsx"); //업로드할 파일필터
    /**
    ...
    private function _loadExistingController()
    {
        ...
        if (file_exists($file)) {
            ...
            //호출 URL이 이미지 파일인지를 체크한다.
            $filename = $this->_url[count($this->_url)-1];
            $ext = strtolower(substr($filename,(strrpos($filename,'.')+1)));
            if ( in_array($ext,$this->arr_tail) ){
                header('location: '.__ENV_NO_IMAGE__);
                return false;
            }else{
                $this->_error("Not found Controller");
                return false;
            }
        }
    }
